...
많아보이지 사실상 select, input 태그들만 만들면 그냥 끝이다
...
그럼 input 창을 디자인을 하고 아이콘만 얹으면 된다
...
2. 사이즈는 (300 \* 30)
   ...
   (예시로 준 사진엔 radius 가 없는데 난 걍 줬다 맘에 안들면 직접와서 바꾸던지 본인이 만들걸로 교체하던지 알아서 해라)
...
하지만 우린 단순히 input 창만 쓸게 아니라 select 로 만들어서 쓸것이다.
...
text 값을 바꾸고 옆에 | 와 다운아이콘을 넣어을려면 구간을 나눌 필요가 있다

![](./images/com-select2.png)

select프레임 안에 프레임 2개를 새로 생성해서 select-left, select-right 로 이름 짓자
...
select-right 를 오토레이아웃하면 정렬이되는데
...
우리 개발할때 클래스 가져다 오버라이딩해서 바꿔쓰고 하지 않는가
...
자 이제 달력 아이콘을 사이즈를 적당히 조절해주고 박아주자

![](./images/com-newCom3.png)

사이즈 조절 옆에 lock aspect ratio 라고 비율에 맞게 사이즈 조절해주는거 눌러주면 편하게 사이즈 조절이 가능하다
...
이걸 라벨을 붙혀서 새로 컴포넌트로 만들어주던지, 오토레이아웃으로 묶어만 두던지 하고

이제 우리 html div 감싸는거랑 똑같이 쌓으면 끝이다

![](./images/com-buildLayout.png)

하나하나 다 그리면 괜히 난잡하기에 생략하겠다

계속 그룹화랑 오토레이아웃으로 묶어주면서 정렬해주면 된다

이 과정도 생략하겠다

다 설명하면 너무 길어진다 이미 충분히 길다

이걸 다 해주면 이렇게 된다

![](./images/com-alignBefore.png)

이렇게 하나하나 쌓아둔걸 모두 선택해주고 오토레이아웃해주면 된다
...
### 2. 오토레이아웃 해주고 리사이징 해주면 내가 설정한값보다 늘어나거나 줄어드는데?
...
> 인스턴스는 수정이 불가능함<br>
> 그리고 인스턴스가 컴포넌트로 바뀌는게 아니라 <br>
> 새 컴포넌트가 만들어지면서 그 안에 인스턴스가 들어가 있는 형태가 되니<br>
> 걍 의미가 없음
